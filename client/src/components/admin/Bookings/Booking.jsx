...
const Booking = () => {
  const { month } = useParams(); // Capture the month from the URL parameter
  const [bookings, setBookings] = useState([]);
  const [search, setSearch] = useState("");
  const [currentPage, setCurrentPage] = useState(1);
  const rowsPerPage = 5;
  ...
  const filteredBookingsByMonth = bookings.filter((booking) => {
    const bookingMonth = new Date(booking.bookedAt).toLocaleString('default', { month: 'short' });
    return month ? bookingMonth === month : true; // Show bookings for the selected month or all bookings if no month is selected
  });

  // Apply the search filter after the month filter
  const filteredBookings = filteredBookingsByMonth.filter((booking) =>
    [
      booking.userId,
      booking.propertyId,
      booking.paymentId,
  ...
  const handleRented = async (id) => {
    const confirmRent = window.confirm("Are you sure to set Rented property to false?");
    if (!confirmRent) return;
  ...
  return (
    <>
      ...
      <div className="mainalladminbook">
        <div className="adminbook-container">
          <h2 className='adminbook-title'>Bookings in {month || "All Months"}</h2>
            ...
            <tbody>
              {currentBookings.map((booking, index) => (
                <tr key={booking._id}>
                  ...
                  <td>{new Date(booking.expiresAt).toLocaleDateString()}</td>
                  <td className='delrent'>
                    <button onClick={() => handleDelete(booking._id)} className="adminbook-delete-btn">Delete</button>
                    {booking.transactionType === "Rent" && (
                      booking.isRented ? (
                        <button className="adminbook-rent-btn" onClick={() => handleRented(booking._id)}>
                          Rented
          ...
          <div className="adminbook-pagination">
            ...
            <span>Page {currentPage}</span>
            <button onClick={() => setCurrentPage(currentPage + 1)} disabled={indexOfLastRow >= filteredBookings.length}>
              Next
            </button>
          </div>
        </div>
