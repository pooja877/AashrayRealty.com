...
const UserApproveProperties = () => {
  ...
  const toggleLike = async (propertyId) => {
    if (!user) return alert('Please login to like properties.');
    try {
      if (likedProperties.has(propertyId)) {
        await fetch('/api/likes/unlike', {
          method: 'POST',
  ...
  useEffect(() => {
    ...
    if (filters.type) filtered = filtered.filter(p => p.propertyType.toLowerCase() === filters.type.toLowerCase());
    if (filters.bhk) filtered = filtered.filter(p => p.bhk === filters.bhk);
    if (filters.transactionType) filtered = filtered.filter(p => p.transactionType.toLowerCase() === filters.transactionType.toLowerCase());
    setFilteredProperties(filtered);
  }, [filters, properties]);
  ...
  return (
    <div className="userapprove-container">
      <button className="filter-toggle-btn" onClick={toggleFilters}>
        {filterVisible ? "Hide Filters ⬆️" : "Show Filters ⬇️"}
      </button>

      <div className={`userapprove-filters ${filterVisible ? 'show' : 'hide'}`}>
        <select name="city" onChange={handleFilterChange} className="userapprove-select">
          <option value="">🏙️ Select City</option>
          <option value="Ahmedabad">Ahmedabad</option>
          <option value="Gandhinagar">Gandhinagar</option>
          ...
          {filteredProperties.length > 0 ? (
            filteredProperties.map(property => (
              <div key={property._id} className="property-card" onClick={() => navigate(`/userproperties/${property._id}`)}>
                ...
                <p className="property-location">{property.address} {property.area}, {property.city} {property.pincode}</p>
                <div className="property-details">
                  {property.propertyType === "Residential" ? null : <span><i className="fas fa-home"></i> {property.bhk}</span>}
                  <span><i className="fas fa-layer-group"></i> {property.floor}</span>
                </div>
              </div>
            ))
