...
export default function UserPromap() {
    const { id } = useParams();
    ...
    useEffect(() => {
        const fetchProperty = async () => {
            try {
                const response = await fetch(`/api/userproperties/getmapSingle/${id}`);
                    ...
                    <Popup>
                        <div className="popcontainer">
                            {property.images?.length > 0 && (
                                <img className="imagepopup" src={property.images[0]} alt="Property" />
                            )}
